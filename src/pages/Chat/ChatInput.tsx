...
import { SendHorizontal, Square, X, Paperclip, Monitor, Mic, FileText, Film, Music, FileArchive, File, Loader2, AtSign } from 'lucide-react';
...
export function ChatInput({ onSend, onStop, disabled = false, sending = false, isEmpty = false }: ChatInputProps) {
  ...
  const handleScreenCapture = useCallback(async () => {
    try {
      // 调用屏幕捕获 API (后端 Python 服务)
      const response = await fetch('http://localhost:5000/capture', { method: 'POST' });
      if (!response.ok) throw new Error('Screen capture failed');
      const data = await response.json();
      // 将截图添加到附件
      if (data.image_path) {
        // TODO: 实现附件添加逻辑
        console.log('截图已捕获:', data.image_path);
      }
    } catch (error) {
      console.error('屏幕捕获失败:', error);
      alert('屏幕捕获失败，请确保屏幕捕获服务正在运行');
    }
  }, []);
  ...
  const handleVoiceInput = useCallback(async () => {
    try {
      // 调用 FunASR 语音识别服务
      const response = await fetch('http://localhost:8000/asr/recognize', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'start_recognition' })
      });
      ...
      alert('语音识别失败，请检查 FunASR 服务是否正常运行');
    }
  }, []);
